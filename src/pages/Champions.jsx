...
function ExecutiveView({ currentUser, executives }) {
  ...
  const top3Order = [ranked[1], ranked[0], ranked[2]].filter(Boolean); // 2-1-3 podium
  ...
  return (
    <div className="space-y-6">
      ...
      {myEntry && (
        <div className="relative overflow-hidden rounded-2xl"
          ...
          <div className="absolute inset-0 opacity-10">
            <div className="absolute top-4 right-8 text-[180px] font-black text-white leading-none">#{myRank}</div>
          </div>
          <div className="relative p-6">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-full bg-shell-yellow flex items-center justify-center text-shell-gray-800 font-black text-xl shadow-lg shrink-0">
                {myEntry.exec?.avatar}
              </div>
              <div>
                <div className="text-xs text-white/60 uppercase tracking-widest font-semibold mb-0.5">Tu posición este mes</div>
                <div className="text-3xl font-black text-shell-yellow">#{myRank} de {ranked.length}</div>
              ...
              <div className="ml-auto text-right">
                ...
                <div className="text-xs text-shell-yellow font-semibold mt-1">
      ...
      <div className="card p-6">
        <h2 className="text-sm font-bold text-shell-gray-700 text-center mb-6">🏆 Top 3 del Mes</h2>
        <div className="flex items-end justify-center gap-3 mb-4">
          {top3Order.map((entry, idx) => {
            const visualRank = [2, 1, 3][idx];
            const rc = rankColors[visualRank];
            return (
              <div key={entry.executiveId} className="flex flex-col items-center gap-2 w-28">
                {visualRank === 1 && <span className="text-xl">👑</span>}
                <div className={`w-14 h-14 rounded-full bg-gradient-to-br ${rc.bg} flex items-center justify-center text-white font-black text-sm shadow-xl ${rc.glow} shadow-lg border-2 ${rc.border}`}>
                  {entry.exec?.avatar}
                </div>
                ...
                <div className={`w-full rounded-t-xl flex flex-col items-center pt-2 border-2 ${rc.border} ${podiumHeights[visualRank]} relative overflow-hidden`}
                  style={{ background: `linear-gradient(to bottom, ${visualRank === 1 ? 'rgba(251,206,7,0.15)' : visualRank === 2 ? 'rgba(148,163,184,0.1)' : 'rgba(180,120,30,0.1)'}, transparent)` }}>
                  <span className={`text-xl font-black ${rc.text}`}>{visualRank}°</span>
                  <span className="text-[10px] font-bold text-shell-gray-600">{entry.total.toLocaleString()} pts</span>
                </div>
              </div>
            );
          })}
      ...
      <div>
        <h2 className="text-sm font-bold text-shell-gray-700 mb-3">Ranking completo</h2>
        <div className="space-y-3">
          {ranked.map((entry, i) => (
            <ExecCard
              ...
              rank={i + 1}
              exec={entry.exec}
              isMe={entry.executiveId === currentUser?.executiveId}
            />
          ))}
        </div>
      </div>
      ...
      {myEntry && (
        <div className="card p-5">
          <h2 className="text-sm font-bold text-shell-gray-700 mb-4">Mis puntos por categoría</h2>
          <div className="space-y-4">
            {[
              { label: 'Propuestas enviadas',    pts: myEntry.proposalsPts, max: 400, value: `${myEntry.proposals} propuestas`, color: '#FBCE07' },
              { label: 'Visitas reportadas',     pts: myEntry.visitsPts,    max: 350, value: `${myEntry.visits} visitas`,       color: '#3B82F6' },
              { label: 'Primeros contactos',     pts: myEntry.contactsPts,  max: 300, value: `${myEntry.contacts} contactos`,   color: '#10B981' },
              { label: 'Cumplimiento volumen',   pts: myEntry.volumePts,    max: 400, value: `${myEntry.volumePct}% de meta · ${(myEntry.volumeL/1000).toFixed(1)}K L`, color: '#DD1D21' },
            ].map(cat => (
              <div key={cat.label}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-semibold text-shell-gray-700">{cat.label}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-shell-gray-400">{cat.value}</span>
                    <span className="text-xs font-black" style={{ color: cat.color }}>{cat.pts} pts</span>
                  </div>
                </div>
                <div className="w-full bg-shell-gray-100 rounded-full h-2.5 overflow-hidden">
                  <div className="h-2.5 rounded-full transition-all duration-700" style={{ width: `${Math.round(cat.pts / cat.max * 100)}%`, background: cat.color }} />
                </div>
              </div>
            ))}
          </div>
