...
import {
  ...
  Clock, ChevronRight, Calendar, Activity, Bell, Filter
} from 'lucide-react';
...
const activityIcons = {
  reunion: { bg: 'bg-blue-50', icon: '🤝' },
  llamada: { bg: 'bg-green-50', icon: '📞' },
  correo: { bg: 'bg-shell-yellow-light', icon: '✉️' },
  actualización: { bg: 'bg-purple-50', icon: '🔄' },
  visita: { bg: 'bg-orange-50', icon: '🏢' },
  nota: { bg: 'bg-shell-gray-50', icon: '📝' },
};
...
export default function Dashboard() {
  const { currentUser, opportunities, alerts, events, forecastUpdated, executives, companies } = useAppStore();
  ...
  // Base: admin sees all, executive sees own
  const baseOpps = isAdmin
    ? opportunities
    : opportunities.filter(o => o.executiveId === currentUser?.executiveId);
  ...
  const recentActivities = myOpps
    .flatMap(o => (o.activities || []).map(a => ({ ...a, companyId: o.companyId })))
  ...
  const pulseItems = [
    { icon: '📉', text: '2 clientes con caída de volumen', color: 'text-shell-red', alertId: 'alert-03' },
    { icon: '⚠️', text: '1 compromiso técnico por vencer', color: 'text-amber-600', alertId: 'alert-02' },
    { icon: '🔔', text: '4 oportunidades sin seguimiento', color: 'text-orange-600', alertId: 'alert-04' },
    { icon: forecastUpdated ? '📈' : '📊', text: `Forecast del mes: ${forecastPct}%`, color: 'text-green-600', alertId: 'alert-06' },
    { icon: '💧', text: `${(totalVolumeL / 1000).toFixed(1)}K L proyectados`, color: 'text-blue-600', alertId: null },
  ];

  const handlePulseClick = (alertId) => {
    if (!alertId) return;
  ...
  return (
    <div className="page-container">
      {/* ─── Hero ─────────────────────────────────────────────────────────── */}
      <div className="relative bg-white rounded-2xl p-6 mb-5 border border-shell-gray-100 shadow-card overflow-hidden">
        ...
        <div className="relative">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {pulseItems.map(item => (
                  <button
                    key={item.text}
                    onClick={() => handlePulseClick(item.alertId)}
                    className={`flex items-center gap-1.5 bg-shell-gray-50 rounded-lg px-3 py-1.5 text-xs transition-all ${item.alertId ? 'hover:bg-shell-yellow-light hover:shadow-sm cursor-pointer' : 'cursor-default'}`}
                  >
                    <span>{item.icon}</span>
                    <span className={`font-medium ${item.color}`}>{item.text}</span>
                    {item.alertId && <ChevronRight size={10} className="text-shell-gray-300" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0 flex-wrap">
              <Filter size={14} className="text-shell-gray-400 hidden sm:block" />
              <select
              ...
              {(filterIndustry !== 'Todas' || filterExecutive !== 'Todos') && (
                <button
                  onClick={() => { setFilterIndustry('Todas'); setFilterExecutive('Todos'); }}
                  className="text-[10px] text-shell-red font-semibold hover:underline"
                >
                  Limpiar
                </button>
              )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        ...
        <div className="card p-5">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 border-t border-shell-gray-100 pt-4">
            ...
            <div className="text-center">
              <div className="text-lg font-bold text-shell-gray-800">47d</div>
        ...
        {/* Forecast chart */}
        <div className="card p-5">
          <h3 className="section-title mb-4">Forecast de volúmenes (L)</h3>
          <ForecastChart />
        </div>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="section-title mb-0 flex items-center gap-2">
              <Bell size={14} className="text-shell-red" /> Alertas inteligentes
            </h3>
          ...
          <div className="space-y-2">
            {unresolvedAlerts.map(alert => (
              <button
                ...
                className="w-full text-left flex items-start gap-2.5 p-2.5 rounded-xl hover:bg-shell-gray-50 cursor-pointer transition-colors"
              >
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${alert.severity === 'Alta' ? 'bg-red-50 text-shell-red' : alert.severity === 'Media' ? 'bg-amber-50 text-amber-600' : 'bg-shell-gray-100 text-shell-gray-500'}`}>
                    {alert.severity}
                  </span>
                  <ChevronRight size={10} className="text-shell-gray-300" />
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        ...
        <div className="card p-5">
          ...
          {topOpps.length === 0 ? (
            <div className="py-6 text-center text-xs text-shell-gray-400">Sin oportunidades con los filtros seleccionados</div>
          ) : (
            <div className="space-y-2">
              {topOpps.map(opp => (
                <div key={opp.id} onClick={() => navigate(`/opportunities/${opp.id}`)}
                  className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-shell-gray-50 cursor-pointer transition-colors">
        ...
        <div className="card p-5">
          <h3 className="section-title mb-4 flex items-center gap-2">
            <Activity size={14} className="text-shell-gray-400" /> Actividad reciente
          </h3>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="section-title mb-0 flex items-center gap-2">
              <Calendar size={14} className="text-shell-gray-400" /> Próximos compromisos
            </h3>
            ...
            {upcoming.map(evt => (
              <div key={evt.id} className="flex items-center gap-3">
                <div className="text-center w-10 shrink-0">
                  <div className="text-base font-bold text-shell-gray-800 leading-none">{new Date(evt.date).getDate()}</div>
                  <div className="text-[10px] text-shell-gray-400 uppercase">{new Date(evt.date).toLocaleDateString('es-PE', { month: 'short' })}</div>
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    {evt.time && evt.time !== '00:00' && <span className="text-[10px] text-shell-gray-400">{evt.time}</span>}
                    {evt.source === 'b2buddy' && <span className="text-[9px] bg-shell-yellow text-shell-gray-700 px-1 py-0.5 rounded font-bold">B2B</span>}
                  </div>
                </div>
              </div>
            ))}
